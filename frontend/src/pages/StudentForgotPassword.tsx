...
const StudentForgotPassword: React.FC = () => {
  ...
  const verifyTokenFromUrl = async (urlToken: string) => {
    ...
    setError('');
    try {
      const response = await connectToBackend('student_verify_reset_token', { token: urlToken });
      if (response.success) {
  ...
  const handleRequestReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await connectToBackend('student_request_reset', { email, universityId: selectedUniversityId });
      if (response.success) {
        try {
  ...
  const handleVerifyToken = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await connectToBackend('student_verify_reset_token', { token });
      if (response.success) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await connectToBackend('student_reset_password', { token, newPassword });
      if (response.success) {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden' }}>
      <ColorBends speed={0.1} colors={['#7e22ce', '#c026d3', '#3b82f6', '#9333ea']} />
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <div style={{
              ...
              boxShadow: '0 8px 16px rgba(126, 34, 206, 0.3)',
            }}>
            ...
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'white', marginBottom: '0.5rem' }}>
              ...
              {step === 'token' && 'Verify Identity'}
              {step === 'verifying' && 'Verifying...'}
              {step === 'linkSent' && 'Email Sent'}
              {step === 'password' && 'Reset Password'}
            </h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
              {step === 'email' && 'Regain access to your student profile'}
              {step === 'token' && 'Enter the reset token from your email'}
              {step === 'linkSent' && 'Check your inbox for a reset link'}
            ...
            {step === 'email' && (
              <motion.form key="email-step" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleRequestReset} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div className="input-wrapper">
                  ...
                  <input type="email" placeholder="Student Email" value={email} onChange={(e) => setEmail(e.target.value)} required disabled={loading} style={{ paddingRight: '1rem' }} />
                </div>
                <button type="submit" className="btn" disabled={loading} style={{ height: '3.2rem', marginTop: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                  {loading ? 'Processing...' : <>Send Link <ChevronRight size={18} /></>}
                </button>
              </motion.form>
            )}
            ...
            {step === 'linkSent' && (
              <motion.div key="sent-step" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: 'center', padding: '1rem 0' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
                  A link has been dispatched to <strong>{email}</strong>.
                </p>
                <button onClick={() => navigate('/student/login')} className="btn" style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  Go to login
                </button>
              </motion.div>
            )}
            ...
            {step === 'token' && (
              <motion.form key="token-step" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleVerifyToken} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div className="input-wrapper">
                  ...
                  <input type="text" placeholder="Paste reset token" value={token} onChange={(e) => setToken(e.target.value)} required disabled={loading} style={{ paddingRight: '1rem' }} />
                </div>
                <button type="submit" className="btn" disabled={loading} style={{ height: '3.2rem', marginTop: '0.5rem' }}>
          ...
          <p style={{ marginTop: '2rem', textAlign: 'center', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
            Remember your password? <Link to="/student/login" style={{ color: 'var(--accent-purple)', fontWeight: 600, textDecoration: 'none' }}>Login</Link>
          </p>
        </motion.div>
      </div>
    </div>
